...
function serialize(inputCas, outputStream, parameters)
    ...
    local videoFrames = parameters["video"]
    ...
    -- local video = {}
    -- local frame_count = 1
    -- for video_frame in string.gmatch(videoFrames, "([^,]+)") do
    --     local f = {
    --         frameId = frame_count,
    --         frameData = video_frame
    --     }
    --     video["videoFrames"][frame_count] = f
...
function deserialize(inputCas, inputStream)
    ...
    local diarization_anno = luajava.newInstance("org.yhcompute.diarization.uima.type.VideoDiarization")
    ...
    if results["modification_meta"] ~= nil and results["meta"] ~= nil and results["diarization"] ~= nil then
        ...
        for i, sentence in ipairs(diarization["sentences"]) do
            local sentence_anno = luajava.newInstance("uima.tcas.Annotation", inputCas)
            sentence_anno::setBegin(sentence["begin"])
            sentence_anno::setEnd(sentence["end"])
            sentence_anno::setSpeaker(sentence["speaker"])
            sentence_anno::setText(sentence["text"])
        ...
        local meta_anno = luajava.newInstance("org.texttechnologylab.annotation.AnnotatorMetaData", inputCas)
        meta_anno:setReference(sentence_anno)
        meta_anno:setName(meta["name"])
        meta_anno:setVersion(meta["version"])
        meta_anno:setModelName(meta["modelName"])
        meta_anno:setModelVersion(meta["modelVersion"])
