...
parser.add_argument('--alreadyEvaluated',      dest='alreadyEvaluated', action='store_true', help="Skips the evaluation and jumps to visualization")
parser.add_argument('--returnJson',            dest='returnJson', action='store_true', help="Visualizes the data as a json instead of a video")
...
def visualization_json_format(tracks, scores, args):
	...
	# Save json file
	with open(args.savePath + "\\speaker_detection.json", "w", encoding="utf-8") as fp:
		json.dump(json_str, fp, indent=2)
...
def main():
	...
	if os.path.exists(args.savePath) and not args.alreadyEvaluated:
	...
	if args.alreadyEvaluated:
		path = os.path.join(args.pyworkPath, "tracks.pckl")
		fil = open(path, "rb")
		vidTracks = pickle.load(fil)
		path = os.path.join(args.pyworkPath, "scores.pckl")
		fil = open(path, "rb")
	...
	if args.evalCol == True:
		...
		quit()
	elif args.returnJson == True:
		visualization_json_format(vidTracks, scores, args)
		quit()
	else:
